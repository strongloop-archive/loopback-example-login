...
PasswordHash.prototype.checkPassword = function (password, stored_hash) {
	stored_hash = stored_hash.trim();
	var hash = crypto.createHash('md5').update(password).digest('hex').trim();

	var verification = (hash == stored_hash);
	console.log('stored hash was ', stored_hash, ' length ' + stored_hash.length, 'type ' + typeof (stored_hash));
	console.log(' hash is ', hash, ' length ' + hash.length, 'type ' + typeof (hash));
